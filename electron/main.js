...
function initLog() {
  try {
    ...
    fs.writeFileSync(logFilePath, `=== 电商视频工坊启动 ${new Date().toISOString()} (${process.platform}/${process.arch}) ===\n`);
  } catch {
    logFilePath = "";
  }
}
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    if (!process.env.HEADLESS_SMOKE) {
      dialog.showErrorBox(
        "电商视频工坊启动失败 / Commerce Video Studio failed to start",
  ...
  mainWindow.webContents.on("did-fail-load", (_e, errorCode, errorDescription, validatedURL) => {
    ...
    dialog.showErrorBox(
      "电商视频工坊页面加载失败 / Commerce Video Studio page load failed",
